...
//                    URL url = new URL(Constants.URL_QUERY_PHONE + phone);
    ...
    private void showImage() {
        ...
        if (Utils.checkFileIsExist(file)) {
            Log.d("FirstActivity", "image file exists");
            ...
            public void run() {
                try {
                    ...
                    if (200 == responseCode) {
                        ...
                        Log.d("FirstActivity", file.getAbsolutePath().toString());
                        FileOutputStream fileOutputStream = new FileOutputStream(file);
                        byte[] buffer = new byte[1024];
                        int len = -1;
                        while ((len = inputStream.read(buffer)) != -1) {
                            Log.d("FirstActivity", "buffer.length:" + len);
                        ...
                        // error
                        Log.d("FirstActivity", "responseCode:" + responseCode);
                    }
                } catch (MalformedURLException e) {
                    e.printStackTrace();
                } catch (IOException e) {
    ...
    private static class MyHandler extends Handler {
        ...
        private final WeakReference<FirstActivity> weakReference;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case WHAT_DOWNLOAD_IMG:
                    FirstActivity activity = Utils.checkNotNull(weakReference.get(), "activity 引用为空！");
                    activity.showImage((Bitmap) msg.obj);
                    break;
                case WHAT_QUERY_PHONE:
                    FirstActivity act = Utils.checkNotNull(weakReference.get(), "activity 引用为空！");
